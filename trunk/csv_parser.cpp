...
/* This is a slightly modified version of the GPL CSV Parser
   found at http://code.google.com/p/csv-parser-cplusplus
*/
...
// this had a bug in it where a comma would still terminate a quoted field
// so "this, is, a string"  would turn into just this followed by is etc.
// Fixed here, but be careful if you are using the other versions of this --untested
void csv_parser::_get_fields_with_optional_enclosure(csv_row_ptr row, const char * line, const unsigned int * line_length)
{
        ...
        if (*line_length > 0)
        {
                ...
                while(char_pos < *line_length)
                {
                        ...
                        if (curr_char == field_term_char && !inquote)
                        {
                                field_end = char_pos;
                                ...
                                if (field_width==0)   //fix for empty lines added
                                  {
                                    CSV_PARSER_FREE_BUFFER_PTR(field);
                                    return;
                                  }
                        ...
                        /* flip state of quote flag if in a quoted string */
                        /* TODO: Should we recognize \" as a legitimate input character? */
                        if (curr_char==enclosed_char) inquote^=1;
                        /* Move to the next character in the current line */
                        char_pos++;
                }
